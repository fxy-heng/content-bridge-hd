...
ContentBridge 将多平台发布拆成五层：

1. 内容输入层：维护统一的原始内容模型，包括标题、正文、标签、封面、目标受众、语气和计划发布时间。
2. 扩展注册层：将内置平台和用户自定义平台合并成统一注册表，保证新增平台可以立即进入选择、适配、校验、导出和模拟发布。
3. 平台适配层：每个平台实现独立适配器，负责标题、正文结构、标签和校验规则。
4. 发布执行层：`publishToPlatforms` 统一判断本地模拟、排期、校验失败和真实后端发布。
5. 真实发布后端：Express 提供账号凭证、公众号 API 发布、浏览器登录和自动化发布。
...
平台扩展被拆成三种层级，避免把演示配置、长期内置适配和真实发布能力混在一起：

| 层级 | 适用场景 | 进入的能力 | 主要文件 |
| --- | --- | --- | --- |
| 配置扩展 | 快速验证新平台规则，例如抖音、今日头条、快手 | 平台选择、通用适配、校验、预览、Markdown 导出、规则导出、模拟发布 | `src/core/adapters.js`、`src/core/platform-presets.js`、`src/core/rules.js` |
| 适配器扩展 | 高频平台需要专属标题、正文结构、标签和提示词 | 专属平台改写、标题备选、AI 提示词、测试覆盖 | `src/core/adapters.js`、`test/adapters.test.js`、`README.md` |
| 发布器扩展 | 平台提供官方 API，或可以稳定浏览器自动化 | 真实发布、登录态检测、失败原因、状态回查 | `src/core/publisher.js`、`backend/routes/*`、`backend/services/*`、`docs/api-contract.md` |

### 4.1 配置扩展流程

页面“扩展更多平台”会收集平台 Key、名称、风格、发布类型、标题上限、标签上限、正文建议字数和封面建议。保存后数据进入 `contentBridge.customPlatforms`，由 `sanitizeCustomPlatforms` 清洗，再由 `getPlatformRegistry` 与内置平台合并。自定义平台使用 `adaptGenericPlatform`，因此不需要写代码也能进入完整工作流。

### 4.2 适配器扩展流程

当一个平台需要更强表达差异时，应升级为内置适配器：

1. 在 `platformMeta` 中添加平台元信息。
2. 在 `builtInAdapters` 中添加专属适配函数。
3. 为标题截断、正文结构、标签限制和校验补充测试。
4. 更新 README 和规则说明，解释平台边界。

### 4.3 发布器扩展流程

真实发布接入必须遵守统一结果契约：成功返回 `status: "success"`，平台要求人工确认返回 `manual_required` 或可解释失败原因，不能把未确认发布伪装成成功。新增发布器时：

1. 在后端新增平台路由和服务模块。
2. 在 `/api/health` 和 `/api/credentials` 暴露平台状态。
3. 在 `src/core/publisher.js` 注册真实发布 endpoint。
4. 在 `docs/api-contract.md` 记录输入、输出、失败边界和权限要求。
...
- 将高频自定义平台沉淀为内置适配器，并为真实发布补充平台 Publisher。
